...
def trainval():
  ...
  """saver"""
  model_variables_prediction = {}
  for variable in tf.global_variables():
    if variable.name.find('LSTM_Prediction') >= 0:
      model_variables_prediction[variable.name.replace(':0', '')] = variable
  print('#####################################################')
  for save_item in model_variables_prediction.keys():
    print(save_item)
  print('#####################################################')
  with tf.name_scope('saver_prediction'):
    saver_prediction = tf.train.Saver(var_list=model_variables_prediction, max_to_keep=cfg.OUTPUT.MAX_MODELS_TO_KEEP)

  model_variables_correction = {}
  for variable in tf.global_variables():
    if variable.name.find('LSTM_Error_Correction') >= 0:
      model_variables_correction[variable.name.replace(':0', '')] = variable
  print('#####################################################')
  for save_item in model_variables_correction.keys():
    print(save_item)
  print('#####################################################')
  with tf.name_scope('saver_correction'):
    saver = tf.train.Saver(var_list=model_variables_correction, max_to_keep=cfg.OUTPUT.MAX_MODELS_TO_KEEP)
  ...
  for save_item in model_variables.keys():
    print(save_item)
  print('#####################################################')
  with tf.name_scope('saver_global'):
    saver_global = tf.train.Saver(var_list=model_variables, max_to_keep=cfg.OUTPUT.MAX_MODELS_TO_KEEP)
  ...
  with tf.Session(config=config) as sess:
    ...
    # 修改成DA-LSTM效果最好模型的路径
    model_file = '/home/poac/lyx/Project/ECA-LSTM/out/models_20/ECA-LSTM.ckpt-82001'
    model_file = cfg.INPUT.JOINT_PATH_TO_ATTEN_MODEL
    saver_prediction.restore(sess, model_file)
    # 修改成Error-LSTM效果最好模型的路径
    model_file2 = '/home/poac/lyx/Project/ECA-LSTM/out/models_21/ECA-LSTM.ckpt-3001'
    model_file2 = cfg.INPUT.JOINT_PATH_TO_ERR_MODEL
    ...
    try:
      while True:
        ...
        _, _lr, _step, _loss_prediction, _summaries, _seq_train, _gt_train \
          = sess.run([train_op, lr, global_step, loss_mse, summary_op, seq_train, gt_train])
        ...
        """测试效果"""
        if _step % cfg.OUTPUT.VALID_SAVE_INTERVAL == 0:
          print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
          ...
          if mean_mse_test < min(history_valid_result_mse):
            ...
            saver_global.save(sess, save_path, global_step=global_step)
