...
class Reader(object):
  ...
  def _setup(self, cfg, mode):
    ...
    # self.files_train = files[0: 0]
    # self.files_test = files[0:1]
    # self.files_train = files[0: 1]
    # self.files_test = files[1:2]
    ...
    for path2file in self.files_test:
      ...
      data_len = len(data)
      num_file_samples = (data_len-(4+2*cfg.INPUT.SEQ_LEN+cfg.INPUT.GT_LEN)+1)//cfg.INPUT.STEP_TEST + 1
  ...
  def read(self):
    ...
    def generator_train():
      ...
      while True:
        for path2file in files_train:
          ...
          begin = 0
          while True:
            if begin + 4+2*cfg.INPUT.SEQ_LEN - 1 > data_len - 1 - cfg.INPUT.GT_LEN:
              break
            else:
              seq = data[begin:begin+4+2*cfg.INPUT.SEQ_LEN]
              gt = np.reshape(data[begin+4+2*cfg.INPUT.SEQ_LEN:begin+4+2*cfg.INPUT.SEQ_LEN+cfg.INPUT.GT_LEN], [-1]).tolist()
    ...
    if self.mode == 'Train':
      dataset_train = tf.data.Dataset.from_generator(generator=generator_train,
                                                     ...
                                                     output_shapes=([4+2*cfg.INPUT.SEQ_LEN, 1], [cfg.INPUT.GT_LEN]))
        ...
        for i in range(len(files_test)):
          ...
          while True:
            if begin + 4+2*cfg.INPUT.SEQ_LEN - 1 > data_len - 1 - cfg.INPUT.GT_LEN:
              ...
              seq = data[begin:begin + 4+2*cfg.INPUT.SEQ_LEN]
              gt = np.reshape(data[begin + 4+2*cfg.INPUT.SEQ_LEN:begin + 4+2*cfg.INPUT.SEQ_LEN + cfg.INPUT.GT_LEN],
                              [-1]).tolist()
              yield np.array(seq), np.array(gt), path2file
              begin += cfg.INPUT.STEP_TEST
