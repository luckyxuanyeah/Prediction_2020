...
def trainval():
  ...
  with tf.name_scope('model_train'), tf.variable_scope('LSTM', reuse=tf.AUTO_REUSE):
    '''主模型前传'''
    prediction_outputs_train = models.prediction_inference_with_attention(cfg, seq_train_for_prediction, is_training=False)  # shape=[batch_size, gt_len]
    print(prediction_outputs_train.get_shape().as_list())
    '''计算所有的error'''
    history_prediction = models.prediction_inference_with_attention(cfg, seq_train_for_history_prediction, is_training=False) # shape=[batch_size*seq_len, gt_len]
  ...
  """variables to train"""
  vars_model = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope='LSTM/LSTM_Error_Correction')
  print(vars_model)
  ...
  with tf.name_scope('train'):
    with tf.control_dependencies(update_ops):
      train_op = opt.minimize(loss_err_correction, global_step=global_step, var_list=vars_model, name='train_op')
  ...
  with tf.name_scope('model_test'), tf.variable_scope('LSTM', reuse=True):
    ...
    corrected_res = err_correction_test + prediction_outputs_test

  """saver loader"""
  model_variables_load = {}
  for variable in tf.global_variables():
    if variable.name.find('LSTM/LSTM_Prediction') >= 0:
      model_variables_load[variable.name.replace(':0', '')] = variable
  print('#####################################################')
  for save_item in model_variables_load.keys():
    print(save_item)
  print('#####################################################')
  with tf.name_scope('saver'):
    saver_loader = tf.train.Saver(var_list=model_variables_load, max_to_keep=cfg.OUTPUT.MAX_MODELS_TO_KEEP)
  ...
  with tf.Session(config=config) as sess:
    ...
    model_file = '/home/poac/lyx/model/20191220_only_attention_0.0018/ECA-LSTM.ckpt-13001'
    saver_loader.restore(sess, model_file)
    # sess.graph.finalize()
    """训练及验证"""
    ...
    try:
      while True:
        ...
        _, _lr, _step, _loss_err_correction, _summaries, _seq_train, _gt_train \
          = sess.run([train_op, lr, global_step, loss_err_correction, summary_op, seq_train, gt_train])
        # _grad = np.array(_grad).shape  # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        """打印结果"""
        if _step % cfg.OUTPUT.PRINT_LOG_INTERVAL == 0:
          end_time = time.time()
          avg_time = (end_time - start_time) / float(1)
          print('Step {} | err_correction Loss: {:.6f}, lr = {:.6f}, time consuming = {}'.format(_step, _loss_err_correction, _lr, avg_time))
          sys.stdout.flush()
        """写入日志"""
        ...
        if _step != 0 and _step % cfg.OUTPUT.VALID_SAVE_INTERVAL == 0:
          ...
          for file_name in list(file_to_num_samples.keys()):
            ...
            for j in range(file_to_num_samples[file_name]):
              _corrected_res, _seq_test, _gt_test, _path2files \
                = sess.run([corrected_res, seq_test, gt_test, path2files])
              res = _corrected_res
