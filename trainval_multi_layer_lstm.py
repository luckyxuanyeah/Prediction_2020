...
def trainval():
  ...
  with tf.name_scope('model_train'), tf.variable_scope('LSTM', reuse=tf.AUTO_REUSE):
    '''主模型前传'''
    prediction_outputs_train = models.prediction_inference_multi_layer_bilstm(cfg, seq_train_for_prediction, is_training=True)  # shape=[batch_size, gt_len]
    # '''计算所有的error'''
    # history_prediction = models.prediction_inference_pure_bilstm(cfg, seq_train_for_history_prediction, is_training=True) # shape=[batch_size*seq_len, gt_len]
  ...
  with tf.name_scope('model_test'), tf.variable_scope('LSTM', reuse=True):
    '''主模型前传'''
    prediction_outputs_test = models.prediction_inference_multi_layer_bilstm(cfg, seq_test_for_prediction, is_training=False)  # shape=[batch_size, gt_len]
    # '''计算所有的error'''
    # history_prediction_test = models.prediction_inference_pure_bilstm(cfg, seq_test_for_history_prediction, is_training=False) # shape=[batch_size*seq_len, gt_len]
