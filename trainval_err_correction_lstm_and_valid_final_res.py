...
def trainval():
  ...
  with tf.name_scope('model_test'), tf.variable_scope('LSTM', reuse=True):
    '''主模型前传'''
    prediction_outputs_test = models.prediction_inference_with_attention(cfg, seq_test_for_prediction, is_training=True)  # shape=[batch_size, gt_len]
    '''计算所有的error'''
    history_prediction_test = models.prediction_inference_with_attention(cfg, seq_test_for_history_prediction, is_training=True) # shape=[batch_size*seq_len, gt_len]
    ...
    '''error correction'''
    err_correction_test = models.err_correction_inference(cfg, err_test, is_training=True)
